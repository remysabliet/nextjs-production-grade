...
export default (req, res) =>
  NextAuth(req, res, {
    session: {
      // use JWTs instead (By default DB Session)
      jwt: true,
    },
    jwt: {
    ...
    // specify we want to use our Signin Page instead the one generate by NextAuth
    pages: {
      signIn: '/signin',
    },
    ...
    callbacks: {
      ...
      async jwt(tokenPayload, user, account, profile, isNewUser) {
        ...
        console.log('JWT Calback isNewUser:', isNewUser)

        if (isNewUser) {
          console.log('JWT Calback OK')
